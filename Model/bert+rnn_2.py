...
class Config(object):
    ...
    def __init__(self, dataset):
        self.model_name = 'bert+rnn*2'
        self.train_path = dataset + '/movie_flc-data/train.txt'                                # 训练集
        self.dev_path = dataset + '/movie_flc-data/dev.txt'                                    # 验证集
        self.test_path = dataset + '/movie_flc-data/test.txt'                                  # 测试集
        # self.class_list = [x.strip() for x in open(
        #     dataset + '/data/class.txt').readlines()]                                  # 类别名单
        self.save_path = dataset + '/saved_dict_movie_flc/' + self.model_name + '+3loss.ckpt'        # 模型训练结果
        self.log_path = dataset + '/saved_log_movie_flc/' + self.model_name + '+3loss.log'           # 日记地址
...
class Model(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.fc_rnn2 = nn.Linear(config.rnn_hidden * 2, config.num_classes)
